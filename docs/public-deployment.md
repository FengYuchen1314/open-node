...
7. 显示初始化地址；直接在可信 HTTPS 页面创建首个管理员，不需要初始化凭证。

脚本只有在全部步骤完成后才退出，并输出 `ACTION_COMPLETE action=install`。看到这行后
再访问脚本显示的 `https://<公网 IP>:58090`。应用明文 HTTP 端口只在宿主回环可用，
不直接暴露到公网。误用 `http://<公网 IP>:58090` 时，Caddy 会返回 `308` 并保留路径
和查询参数，自动跳转到相同端口的 HTTPS 地址。
管理员创建前，首位访问初始化页面的人可以取得管理员权限，请在安装完成后立即初始化。
