...
默认 Docker Compose 的 SQLite 数据布局支持此入口。管理员打开“备份与恢复”，选择本项目
...
完成解密、归档/摘要、SQLite 完整性/外键、管理员和密钥依赖检查，并清除旧会话、撤销安装
票据及隔离自动任务。任何准备失败都不会覆盖当前实例。

准备成功后会写入可恢复的激活日志。安装器管理的 Compose 容器在响应返回后自动重启，
入口脚本在 SQLite 打开前把旧顶层状态移动到
`.open-node-restore-rollback-<上传请求 UUID>/`，再安装已经验证的恢复树；中途退出会按日志
继续，而不是混用两套数据库。自定义部署默认不自动终止进程，须由操作者重启。不要在服务
运行时删除等待激活目录、激活日志或回滚目录；确认恢复实例稳定并另有外部备份后，才可在
停服状态人工归档/清理旧回滚树。

重启后仍进入下文“首次启动复核”隔离。浏览器入口只支持默认同一私有数据根的 SQLite
布局；自定义分散目录和 PostgreSQL 当前会显示不支持，继续使用受控离线流程。
...
不接受原版 mmwx ZIP、安装器 tar 或 PostgreSQL dump；数据库之间的迁移尚未实现。
