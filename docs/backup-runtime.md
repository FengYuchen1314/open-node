...
状态：写入协调、数据库与状态快照、依赖检查、加密创建器及网页任务已实现。
最新一次后端目标门 512 项通过，零跳过，含真实 HTTP 创建、下载和官方 age 独立解密。
历史回归和修正记录见[测试说明](testing.md)。发布检查以对应提交的 GitHub CI 为准，生产实例没有升级。

本页描述内部服务；网页任务、身份验证、限时下载与限制见[网页备份用法](backups.md)。
现有 `open-node-backup encrypt` 仍只加密已经完成的 v1 ZIP，不能代替这里的快照流程。
管理员创建任务、重新验证身份和下载期间的撤销检查已接通；受控恢复仍待实现。
