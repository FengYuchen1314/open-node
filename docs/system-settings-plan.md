...
状态：首期已在 `f0ed515d71aa28f097d43f469d6fcc232859af9e` 发布到 `main`。
完整回归、真实浏览器、精确 Git 镜像和四项 CI 均已通过，生产未升级。
这不是通知提交 `bf8eaa8` 的功能。用法见[站点文字说明](system-settings.md)。
...
当前实现已让 SQLite 与 PostgreSQL 共用相同的版本化持久化合同；品牌存储不可用仍不能
阻断其余应用启动。
