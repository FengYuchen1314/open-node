...
节点创建、导入、编排和测速统一位于管理端“节点管理”。Open Node 把“面板新建受管节点”
和“导入已有节点”分开处理：
...
启用网站入口前，先从反代界面的“管理可信证书”进入证书管理，签发覆盖网站 SNI 的受信
证书，并为当前服务器添加和执行部署目标。部署目标的证书文件名必须与反代表单中的
“证书名称”完全一致；共享入口会在应用配置前验证证书与私钥以及 SNI，不会自动改用
自签证书或跳过校验。

保存和禁用都使用配置 revision 做并发检查，并生成 Agent 命令。页面显示“声明已保存”
不等于主机已经生效，必须继续确认命令状态为“应用成功”；失败或未执行时应先查看命令
结果和 Agent 日志，排除问题后可用“重新下发”重试当前声明。
...
- 已确认的外部订阅节点也可作为候选，支持纯外部和受管/外部混合线路。同一线路中的外部
  节点必须属于同一用户，所得逻辑节点也只能通过该用户的主订阅渲染；外部密钥不会出现在候选 API 中。
- 保存时后端再次执行相同检查。编辑和删除使用 revision；删除还要求输入完整名称。

有效编排保存为可供套餐选择的逻辑节点。节点停用或消失后，包含它的草稿会提示移除，
不能继续保存。Clash/Stash 按轮询组输出，Xray 使用原生 `roundRobin` balancer；
sing-box 1.14 不提供轮询组，故降级为标准 `urltest` 自动低延迟/故障切换。
