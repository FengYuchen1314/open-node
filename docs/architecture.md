...
through `/api/v1/servers/{server_id}/xray/runtime/nodes`. Those drafts copy
public connection metadata only; per-user credentials are still generated by
the subscription system. Scan records are operational health data only; they do
not contain or imply licensing state.
