...
absent from runtime; extra runtime clients are reported but not removed
automatically. Those drafts, drift records, sync writes, and credential checks copy
public connection metadata only; per-user
credentials are still generated by the subscription system. Scan records are
