...
sends per-email `remove-client` commands to the matching inbound. Those drafts,
drift records, sync writes, and credential checks copy public connection metadata only; per-user
