...
- 共享服务器现已进入普通服务器、流量、DDNS、探针、运行时节点和订阅凭据流程；五秒
  后台同步不会推进管理修订号，网络读取不占用备份写租约，失败保留最后成功快照并退避。
- 后端共享、官方线格式、订阅访问、应用/HTTP 备份协调、身份隔离与 secure-channel
  聚焦回归全部通过；其中最终组合门超过 **230 项通过**，变更文件 Ruff 通过。
- 固定官方临时 X25519、分享令牌 HMAC-SHA256、`securechan-v1` HKDF 与 AES-256-GCM
  往返、重放拒绝、令牌不匹配、会话失效重协商均有测试；Open Node 消费官方拥有方及
  官方消费方调用 Open Node `/api/federation/*` 两个方向均覆盖。
- 中文配置工作区 **26 项通过**，分享服务器仅显示节点和凭据范围；前端类型检查与生产
  构建通过。部署资产、首次初始化和公网入口 **41 项通过**，继续覆盖 GitHub 脚本新装的
  关键契约。商业 Reality 池、许可证逻辑及生产升级不在范围内。
- 联调复核补齐前端对新增 `nginx` / `probe_sys` 的严格解析；共享服务和页面 **6 项**、
  Dashboard Nginx 定点 **2 项**、Agent Nginx/传输 **38 项**、后端扫描/共享 **7 项**通过。
  Agent 0.3.0a2 从精确提交 `72b38c0` 构建，wheel SHA-256 为 `cd072ace…`；发布前真实
  systemd 门及发布后匿名 GitHub WebSocket/HTTP 下载、升级、VLESS 流量、回滚与清理
  全部通过。四个公开资产的 GitHub 摘要与构建记录一致，生产没有升级。

## 自定义规则、Proxy Provider 与覆写脚本 — 2026-09-01

隔离候选位于 VPS `/tmp/open-node-permissions.tYdQQr/repo`，没有修改生产
`/opt/open-node`、生产数据库或容器。

- 后端自定义规则/Provider 4 项及外部订阅定向回归 48 项分别通过，合计 **52 项通过**；
  变更文件 Ruff 通过。覆盖三类 YAML、替换/前置/追加、重复键拒绝、所有者边界、订阅
  渲染、公开快照、短码/IP 策略、无上游泄露、删除资源和删除来源时的引用清理。
- 新增来源级联用例第一次按不完整删除载荷请求，接口在事务前正确返回 422；测试按既有
  `confirm: true` 契约修正后 4 项全部通过，产品代码没有为夹具放宽确认门槛。
- 前端规则/Provider 服务、管理员/用户页面、用户中心和订阅配置回归共 **32 项通过**；
  类型检查及主应用生产构建通过。构建只有既有的大 chunk 提示。
- 后续脚本批次的后端脚本/规则/订阅定向回归 **19 项通过**，覆盖 QuickJS 语法检查、
  `produce()`、执行顺序、硬超时、错误不回显、账户隔离、档案选择、无效结果跳过及实际
  Clash 渲染；变更文件 Ruff 通过。
- 前端脚本服务、订阅自定义页、账户页、主导航、订阅档案及相邻服务累计 **48 项通过**，类型检查和
  主应用生产构建通过。隔离 Docker 从当前源码成功安装 `quickjs==1.19.4`、构建前端并以
  非 root 运行时实际执行 Hook；测试镜像随后删除，生产未升级。
- 高级 Provider 聚焦后端 **7 项通过**，覆盖 Mihomo 请求头、服务端同名代理组、名称与
  GeoIP 并集、排除优先、无 token 拒绝、敏感错误不回显和服务端模式公开路由 404；变更
  文件 Ruff 通过。前端对应服务/页面 **4 项通过**、类型检查和生产构建通过；安装器无容器
  烟测 **55 个场景通过**。GeoIP 使用固定模拟国家结果，没有发送真实节点地址或使用真实
  IPinfo token。
- 没有重复历史全套或真实浏览器。上一批公开 CI 是 4,997 通过、116 跳过，唯一失败是
  Worker 协调夹具漏列后来加入的 `DDNSWorker`；夹具已补齐并连续 5 次定点通过。
...
- 新增后端 DDNS **4 项通过**；连同服务器创建、Agent 注册/心跳和证书凭据删除保护的
  定向回归共 **6 项通过**。变更文件 Ruff 通过。
- 覆盖六类供应商工厂与固定端点、复合公共后缀域名、A/AAAA、显式/自动服务商、加密凭据
  复用、IP 漂移、手动强制、租约/修订栅栏、固定错误及远端正文不回显。
- 前端严格服务契约 **3 项**、中文 Ant Design 页面 **2 项**，合计 **5 项通过**；类型检查
  通过。覆盖状态读取、带修订保存、手动排队、固定错误和畸形成功响应拒绝。
- 生产构建通过，只有既有的大 chunk 提示。没有使用真实供应商账号写公网 DNS；该项需要
  操作者提供自己的域名和最小权限凭据，不能把模拟契约门称为六家实网验收。

## 服务器共享与联邦首版（历史门）— 2026-09-01
...
- 本批没有重复历史全套、真实浏览器、Docker 新装和生产升级。当时接入服务器尚未进入
  普通 inventory/node catalog，官方可选消息加密与反向旧协议互通也不在该历史门范围；
  这些差异已由本文件开头的新批次补齐。
- 发布提交 `df38bd5` 的 GitHub Backend 全套为 **4,984 通过、116 跳过、3 失败**；三项
  都是公开命令查询的占位 UUID 在分享令牌前返回 422，而全局认证枚举要求未认证先返回
  401。路径参数改为令牌检查后解析，并新增回归；共享 5 项及原三项认证枚举合计
  **8 项通过**。Frontend、Agent 和 Probe Worker 原本均成功。

## 用户功能权限与配额 — 2026-09-01

隔离检出位于 VPS `/tmp/open-node-permissions.tYdQQr/repo`，基于公开
`a79ca2376bdc6caa5a584c337d7bec4b1d03b83d`，只叠加本批权限源码和测试；没有修改
生产 `/opt/open-node`、共享候选、生产数据库或容器。

- 后端 Ruff 通过。权限专项、订阅模板、用户外部订阅、私有路由和续费共
  **73 项通过**，退出 0；新增权限专项 **5 项通过**。
- 新增门槛覆盖默认开放、管理员版本保存、用户用量、四组账户 API 固定 403、
  管理员配额旁路、严格有界请求，以及模板/外部来源同时创建不能超过上限。
- 前端权限服务、管理面板、账户中心和系统设置回归 **44 项通过**；类型检查和
  主应用生产构建通过。权限读取失败时只保留订阅与安全页，不显示原始错误。
- 本批未重复历史全套、真实浏览器和 Docker 安装门槛；它不改变已经通过的 GitHub
  新装、网页内更新、Agent、Probe 或生产实例证据。

## 网页内应用更新 — 2026-09-01

候选位于 VPS `/tmp/open-node-app-update.2T4ehP/repo`。其中临时提交
`e258099c6cd77cb142beb3b6717e71544b9fcdd9` 只用于让安装器烟测从 Git 克隆完整候选，
不是公开发布提交。生产 `/opt/open-node` 和共享候选均未升级。

功能提交 `ff010413f4a163a4f24493039ca9d6f052a1643e` 随后快进发布到官方 `main`。
使用公开 Raw 安装器在 `/root/open-node-web-update-e2e-pfH7xWRr` 完成官方仓库、`main`、
systemd 主机的独立新装；项目 `on-web-update-e2e-pfh7xwrr` 监听回环端口 18082。manifest
记录精确 `ff01041…`，容器健康，更新 path unit 为 active/enabled。维护目录和状态分别为
`1770 root:10001`、`0640 root:10001`；实际容器只有数据卷及固定维护 bind，初始状态为
`idle`。首次尝试把私有配置放在 `/tmp` 时，安装器因全局可写父目录在创建资源前拒绝，
随后改用 root 私有目录完成安装。

纯文档后继提交 `285c0517691280929d5187ab73b2df70bcee56ec` 发布后，使用该实例的
管理员会话调用 Web API。`check` 请求 `d4566586-957a-4941-b2e0-3859d9aafb2e`
从 `idle` 返回 `available`，目标精确等于该提交；`apply` 请求
`08aab058-fe80-46da-95a4-58c8fad78650` 从旧版本进入 `updating`，跨容器重启后返回
`succeeded`。最终状态、manifest、环境、源码 HEAD、容器镜像 ID 和 OCI revision 均为
`285c051…`，镜像健康；更新创建了以 `ff01041…` 为旧版本的私有备份，没有恢复标记，
systemd watcher 保持 active/enabled。隔离实例和生产 `/healthz` 同时正常，管理员会话
在更新前后保持有效。该流程证明的是官方 `main` 上真实的 Web 检查、固定目标确认、
宿主备份/构建/重启及完成回执，不是直接调用助手或 CLI 代替 Web 写请求。

- 后端 Ruff 通过；应用更新 API/文件边界与部署资产共 **15 项通过**。
- 安装器 Bash 语法通过；首次初始化、公网入口及部署资产共 **41 项通过**。
- 前端服务、应用更新面板和系统设置回归共 **29 项通过**；App 类型检查和生产构建通过。
  构建只有既有的大 chunk 提示。
- 根助手在 `/tmp/open-node-update-helper.1pU1nh` 消费 `10001:10001`、`0600` 的固定
  `check` 请求，查询官方 `main`，写回 `root:10001`、`0640` 的状态并删除请求；结果显示
  当前与目标均为公开提交 `81d26860158eaf36580d32d5781fb07aaa8a46d6`。
- 实际 `docker create` 证明候选容器只有既有数据卷和精确的
  `/var/lib/open-node-maintenance-<project>:/run/open-node-maintenance:rw`，没有 Docker socket、
  源码或私有配置挂载。

完整安装器烟测首轮使用随机项目 `on-inst-897f8ba007ba`。它通过目标提交绑定、停止容器
备份、挂载/清单/镜像漂移拒绝和卷更新，随后在注入备份失败后重新登录时返回 HTTP 500。
清理后磁盘只剩 1.1 GiB；生产健康，随机容器、网络、卷、镜像和维护目录均已清理。
只删除 20 个历史 `/tmp/open-node-*` 证据目录中的可重建 `.venv`/`node_modules` 后，
源码、日志、数据库和 Docker 数据保留，可用空间增至 6.7 GiB。

第二轮随机项目 `on-inst-5b3a3effa4f8` 完整通过，包括：

- 缺失数据卷和资源标签漂移拒绝；
- 候选启动后中断的隔离、备份恢复及宿主根挂载拒绝；
- 新装、管理员登录、并发锁和同提交无变化更新；
- 目标提交变化时在停止服务和创建备份前拒绝；
- 停止容器更新、不可变备份恢复、环境/标签/清单/容器/挂载漂移拒绝；
- no-op 保留回滚物，卸载/重装保留管理员数据；
- 卷更新、注入备份失败后原服务恢复登录、构建中断保持部署身份；
- 修改数据但健康失败的候选进入双重隔离，使用备份恢复原登录；
- 最终清理全部随机容器、网络、卷和镜像。

同一点在有 6.7 GiB 余量时通过，确认首轮 HTTP 500 来自测试峰值下的磁盘写入不足，
不是本批应用更新回归。测试没有调用生产更新助手，也没有修改生产数据。

## Web 公告 — 2026-09-01

VPS 验证目录为 `/tmp/open-node-announcements.SUpJlc/repo`。测试使用临时源码、临时
SQLite 数据库和模拟会话，没有修改 `/opt/open-node`、共享候选或生产数据。

- 后端 Ruff 通过；公告存储、权限、套餐过滤、到期、删除、严格请求和固定错误共
  **10 项测试通过**。
- 前端 TypeScript 检查通过；公告服务和管理组件 **20 项测试通过**，用户中心相邻
  回归 **14 项通过**，系统设置既有 **23 项通过**。
- 主站生产构建通过。公告标题和正文的纯文本渲染、显式发布确认、单次写入、删除、
  不确定结果只读核对，以及公告读取失败不阻塞账户均有定向覆盖。

没有重复历史全套、真实浏览器或 Docker 镜像验收，也没有测试 Bot/Mini App 和自动
节点阻断广播；这些能力不属于本期 Web 公告范围。
...
提交 `1691e62` 的 CI 中 Frontend、Agent 和 Probe Worker 成功；Backend 完成全套后为
**4,954 通过、116 跳过、7 失败**。七项都来自公网安装器单元夹具：真实安装器要求
root 所有，但 GitHub checkout 和 `tmp_path` 由非 root runner 所有，测试在进入目标
分支前即被安全检查拒绝。产品检查未放宽；单元夹具改为保留普通文件、非符号链接、
私有权限和不可组/全局写检查，只把预期所有者替换成当前隔离测试账户。修正后，同一
25 项测试分别以 VPS root 和 `nobody` 账户全部通过。真实 root 所有权仍由安装器静态
检查和 root 运行的 VPS 策略烟测覆盖。
