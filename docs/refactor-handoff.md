...
同步主线；`9602217` 的候选与主线 CI 也均四项通过。当前工作树继续实现备份加密：
...
而汇总为 1，实际测试和独立后核均通过，原失败记录保留。精确提交镜像与发布验收
仍待完成，尚未把这一加密切片发布主线。网页创建/下载、一致快照和受控恢复仍未实现。
...
格式工具与只读 CLI，数据库、密钥、来源认证、一致快照和恢复仍明确未验证。
[`backup-plan.md`](backup-plan.md) 保留已核对的官方差异、全部写入路径和后续恢复
边界，接下来应完成实际快照、加密制品和受控恢复，不要停在格式层或重复既有冷备。
...
| 项目 | 当前状态 |
| --- | --- |
| GitHub `main` | 已发布备份格式层和只读 CLI `2a2810325e092b760481e427ea500245d01a3884`，此前站点文字 `f0ed515`、通知首期 `bf8eaa8`、外部订阅/中文界面 `998839b`、React/Ant Design、面板 Agent 安装等功能保持。文档或 test-only 后续提交不改变功能基线 |
| 备份格式发布 | `2a28103` 的 785 项专项、真实大包和完整后端 3285 项通过；精确 Git 镜像 `sha256:642bc4a8ee1069aaa87aed00ff8291fde34c3e38a66acd03c5678ac641cb9d43`、18 项实际 CLI/守卫检查、独立新应用及候选四项 CI 全部通过。完整后核和原失败记录见 `testing.md`。网页备份、认证加密、在线快照、受控恢复和完整迁移未实现 |
