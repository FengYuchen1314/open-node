...
用户已明确：只考虑新部署，不做旧妙妙屋迁移，不接入 Bot。后续不再开发旧 mmwx
...
统计窗口、临时封禁时长和本地 IP 跳过策略。默认保持官方 5 次/24 小时后封禁 24 小时；
生效封禁持久化，未达阈值计数只在内存，解封保留历史。公开订阅只记录固定路由模板，
不保存实际 Token、短码、密码或正文。候选代码为 `9eba59d`；隔离 Linux 新增后端 6 项、
认证/订阅/备份协调邻近 241 项、前端新增 6 项及邻近 69 项通过，Ruff、类型检查和生产
构建通过。生产 `/opt/open-node` 未修改。Turnstile、通用请求速率、未知 UA、封禁通知和
多管理员 RBAC 留在后续安全批次；用法见 [security-management.md](security-management.md)。

最新候选按固定官方 `setup.go` / `profile.go` 补齐首次初始化资料。中文初始化页可选填写
管理员昵称、邮箱和 HTTPS 头像地址，资料与管理员密码、品牌、永久初始化标记及一次性凭证
消费在同一 SQLite 事务提交；空昵称回落为用户名。系统设置新增资料读取与版本化保存，
旧草稿冲突不覆盖新值，结果不确定时只重新读取。头像不由服务端抓取，且拒绝 HTTP、
本地文件、内嵌账号密码和控制字符。VPS 隔离 Linux 初始化文件 **35 项通过**，新增资料
聚焦 **7 项通过**；前端相关 **34 项通过**、Ruff、类型检查和生产构建通过。生产
`/opt/open-node` 未升级。PostgreSQL、DNS 账户和浏览器上传恢复仍是后续独立工作。
...
同步自己名下的入站后可创建普通节点，套餐用户凭据经拥有方联邦接口下发，消费方仍不能
直连 Agent、控制服务、编辑服务器或二次分享。配置工作区对分享服务器只显示节点和凭据
范围。主控间管理请求已实现官方 `X-Fed-KeyEx` / `X-Encrypted`：临时 X25519、令牌绑定
...
复用订阅短码与 IP 策略，不泄露上游地址，也不会在客户端下载时访问上游。管理员和用户
均有中文 Ant Design CRUD；覆写脚本支持官方 `post_fetch`、`pre_save_nodes`、`produce()`、
排序执行和订阅档案选择，运行在有时间/内存/输入输出限制的独立 QuickJS 子进程。无效脚本
...
显式或证书优先的自动服务商、手动同步、状态和五分钟失败重试。后台以持久的上次成功 IP
检测漂移，并用 75 秒租约、修订号和备份写协调恢复任务。中文 Ant Design 页面已经加入
主导航。VPS 隔离候选的 DDNS 后端 4 项、服务器/证书相关 2 项、前端 5 项、Ruff 和类型
检查通过；供应商请求用固定模拟响应，没有操作者真实 DNS 账号，因此不声称六家公网写入
均已实账验收。使用和边界见 [ddns.md](ddns.md)。生产 `/opt/open-node` 未升级。

DDNS 现在也能使用分享服务器自动同步的 IPv4/IPv6 投影，但仍不负责给面板配置 DNS、
开放防火墙或签发公网证书。真实供应商写入需要操作者账号验收。

当前批次完成服务器共享/联邦首版。拥有方可为本地 Agent 服务器签发只显示一次的令牌，
选择有限入站权限或完整的已支持 Agent 权限，吊销时可排队清理该分享创建的入站；接入方
通过公开 HTTPS 导入、加密保存令牌、查看/刷新快照并提交和轮询耐久命令。主控间连接固定
公开解析地址、验证 TLS/SNI、拒绝重定向/私网目标和超大响应。中文官方 Ant Design 页面
覆盖创建、复制、吊销、接入、刷新、移除与命令工作台。具体边界见
[server-sharing.md](server-sharing.md)。VPS 隔离候选通过新增后端 5 项、前端 5 项、Ruff、
类型检查和生产构建；生产 `/opt/open-node` 未升级。

共享提交 `df38bd5` 的 GitHub 全套只在公开联邦命令占位 UUID 的认证先后顺序失败三项：
4,984 项通过、116 跳过，Frontend/Agent/Probe 均成功。现已把令牌校验放在 UUID 解析前，
共享 5 项与原三项认证枚举共 8 项通过，未认证请求重新稳定返回 401。

上述“首版”差异已由文件开头的最新候选取代：普通资产/节点目录、后台自动刷新、官方
消息层加密和反向旧路径互通均已补齐。仍不复制 Reality 商业共享池和许可证逻辑，软件
继续免费。

本轮按固定官方 `user_permissions.go` 完成普通用户功能权限与配额。系统设置可统一
开放个人模板、外部订阅、私有路由和续费，模板/外部来源支持每用户数量上限；关闭后
四组账户 API 固定返回 403，数量检查与原创建写事务一致。用户中心只显示开放入口并
显示用量，权限读取失败时保留订阅和安全页。VPS 隔离检出
`/tmp/open-node-permissions.tYdQQr/repo` 的 Ruff、73 项相关后端、44 项前端、类型检查
和生产构建通过；生产未升级。下一步进入服务器共享/联邦。

上一轮已完成网页内应用更新候选。管理员在中文系统设置页检查官方 `main`、核对完整目标
...
解码和摘要寻址，二进制保存在 SQLite 并随正常数据库快照。没有照搬官方像素/动漫
皮肤，不引入 PRO 或许可证门槛。用法和差异见 [appearance.md](appearance.md)。

VPS R1 后端 210 通过、2 个测试预期失败；前端 82 通过、2 个测试断言失败，App/Node
类型检查和主站构建通过。修正夹具并统一前后端 HTTPS URL 边界后，R2 Ruff、后端 27
项、前端 42 项和 App 类型检查全部通过，证据保留在
`/tmp/open-node-appearance-r2.WhxKJy/evidence`。最终 R3 把损坏动图 EOF 固定为无效图片
响应，并让菜单显式跟随 Ant Design 深浅主题；Ruff、后端 28 项、App 15 项及类型检查
...
本轮补外部订阅定时刷新：管理员与用户共用中文设置页，可选仅更新已保存节点或自动
加入新节点，间隔 15 分钟至 7 天。持久化租约防重复，来源版本防止旧结果覆盖新设置，
抓取/解析失败保留节点并退避；更换 URL 和恢复备份均关闭自动刷新。验证记录见
[testing.md](testing.md)，用法见 [external-subscriptions.md](external-subscriptions.md)。
VPS `/tmp/open-node-refresh.e6s5c4WF/evidence`：后端 127 项、前端 89 项通过；
App/Node 类型检查、主站构建通过；初次 Ruff 的格式问题已修正，最终后端 Ruff 通过。
后续优先通用设置、浏览器初始化和新部署流程，再继续 DDNS、共享、权限及规则等缺项。
本批不是整个目标完成，生产与共享候选不自动升级。

上一版 `74a98e3` 对应完整 CI [33416036932](https://github.com/FengYuchen1314/open-node/actions/runs/33416036932)
已结束，Backend、Frontend、Agent、Probe Worker 四项全部成功。此结果取代此前“仍在运行”的快照。

## 历史批次记录

本轮新增 v1 离线受控恢复：`open-node-backup restore` 导入新私有目录，校验数据库/密钥，
清除旧会话、撤销安装票据、终止待执行命令和证书任务，隔离旧证书任务文件。
...
前端创建/恢复页面和服务共 22 个不同用例通过，类型检查通过；主站构建在 R1 通过。
未做浏览器全流程或新 Docker 镜像重验，未升级生产或共享候选。
...
具备 IPv4/IPv6、搜索分页、部分/不支持/未配置/失败状态和按接收时间过期；IP 仅管理员
可读，不是设备数。VPS 上 19 后端、20 在线采集 Agent、11 相邻 Agent、12 页面测试
通过，含真实 VLESS 连接建立/关闭后的 IP 变化，类型检查及主站构建通过。
旧 CI `33408662933` 的 5 个前端失败已修正测试夹具，25 项定点回归通过；后端在
75% 处碰到 30 分钟上限，没有完整结论，已将该任务上限调为 60 分钟，未删测试。
发布标签不再触发重复全套 CI，主线发布仍触发全套。新版本构建资源与说明见
[`releases/agent-0.3.0a1.md`](releases/agent-0.3.0a1.md)，面板安装器已改为固定该版本。
专项记录在 `/tmp/open-node-online.JUz2LGwv/evidence`、`evidence-r2`，四个发布资产在
同目录 `assets`。未升级生产或共享候选。下一步继续剩余恢复模式、Bot/Mini App 等缺项，
不要把这一批功能交付当作整个 goal 已完成。

上一批源码已加入网页加密备份、六种客户端输出、URI/Base64 外部订阅、用户自助来源、
...
生产实例没有升级。受控恢复、Bot/Mini App、DDNS 及完整迁移仍待完成。
旧 Agent 0.3.0a0 资产保留不变；IPv6 字面地址证书校验修正已打入 0.3.0a1。
...
[`backup-runtime.md`](backup-runtime.md) 和 `testing.md`。`ef93676` 精确 Git 镜像已通过；
完整回归和 CI 暴露的测试夹具权限、信号及连接清理问题已修，原失败保留。
网页创建/下载随后完成，上述 512 项联动门已通过；受控恢复仍未实现，生产没有升级。
