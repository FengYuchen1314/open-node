...
| P1 | Server sharing/federation | Free owner and consumer workspaces implement one-time hashed tokens, limited/full scopes, inbound ownership, revocation cleanup, encrypted imported secrets, bounded HTTPS transport and durable commands. Imported servers are ordinary inventory, traffic, DDNS, probe and node-catalog assets; five-second refresh, user-credential relay, official X25519/HMAC/HKDF/AES-GCM negotiation, plaintext downgrade and reverse `/api/federation/*` interop are implemented; see [server sharing](server-sharing.md). | Passed the focused owner/consumer, official wire-format, subscription-access, backup-coordination, role-isolation and secure-channel gates. Commercial Reality pools and license enforcement are intentionally absent. The Agent scan does not currently report a separate Nginx version string. |
...
| --- | --- | --- | --- |
| Server/Agent inventory and telemetry | Implemented for local and federated servers. [DDNS](ddns.md) covers both; [server sharing](server-sharing.md) projects owner status, traffic, Xray/Nginx scan state and CPU/memory/disk/network telemetry into the ordinary inventory and node catalog. | The separate home speed tester and commercial Reality resource pool remain absent. Nginx version display awaits an Agent scan field rather than inventing a value. | P2 |
