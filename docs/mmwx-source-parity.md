...
| Setup/backup/database | CLI administrator creation and installer-level stopped-volume backup implemented. [Backup v1 tools](backup-format.md) add manifest/ZIP checking, an internal staging writer and a Chinese read-only validation CLI. Published `a29345b` adds pinned official age encryption of completed archives, read-only authenticated decryption/validation and no-overwrite file publication; the [source-reviewed plan](backup-plan.md) still separates these tools from snapshot or restoration readiness. | Consistent online snapshot, browser setup, downloadable Web backup/restore, SQLite-to-PostgreSQL migration and application update UI remain absent. | P1 |
