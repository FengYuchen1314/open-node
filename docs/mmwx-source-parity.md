...
| Notifications/renewals | Unpublished first slice adds administrator Telegram configuration, preview/test, durable package-expiry reminders and Chinese delivery history; [operator guide](notifications.md) and [source-reviewed design](notifications-plan.md) describe the boundary. Backend 2298, frontend 890/72 files, both builds, real browser and working-tree Docker gates passed; exact-commit Docker and CI remain pending. | User binding, Bot/Mini App, daily digest, threshold/server/security alerts, announcements and renewal approval are absent. No payment or automatic extension. | P1 |
