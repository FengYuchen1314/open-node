...
| Setup/backup/database | CLI administrator creation and installer-level stopped-volume backup implemented. [Backup v1 tools](backup-format.md) add manifest/ZIP checking, an internal staging writer and a Chinese read-only validation CLI; this is only the format layer in the [source-reviewed plan](backup-plan.md), not snapshot or restoration readiness. | Consistent online snapshot, authenticated encryption, browser setup, downloadable backup/restore, SQLite-to-PostgreSQL migration and application update UI remain absent. | P1 |
