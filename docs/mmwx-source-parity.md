...
| P1 | External subscriptions | Administrator-owned workflow implemented for a subscriber's HTTPS Clash/Mihomo YAML sources, encrypted snapshots, explicit preview/confirmation and primary-link merging. | English and Chinese full-suite/Docker gates passed; R4 external browser/Mihomo/Xray VLESS, rotation/key restoration, broader client/template reruns and clean exact-commit image also passed. Published on `main` at `998839b` after all four CI jobs passed. URI/Base64 input, scheduling, subscriber self-service and the provider/rule ecosystem are separate. |
...
| Managed subscriptions | Users, plans, nodes, credentials, quota/reset, templates, profiles, temporary links, access reconciliation and source-compatible frozen accounting implemented; [external sources](external-subscriptions.md) add a published administrator/manual-YAML primary-link workflow | Wider external input, automatic refresh, subscriber self-service and provider/rule/script ecosystem absent. | P1/P2 |
...
| Setup/backup/database | CLI administrator creation and installer-level stopped-volume backup implemented. [Backup v1 tools](backup-format.md) and published `a29345b` provide manifest/ZIP checking, official age encryption and read-only validation. The unpublished [runtime candidate](backup-runtime.md) now coordinates writers and creates private consistent SQLite/state snapshots, checks key dependencies and yields completed ciphertext. Writer-only full regression: 4412 passed; new snapshot/creator integration: 410 passed, zero skips. | Full regression including the new snapshot code, exact Git image and release CI are pending. Browser setup, administrator backup jobs/downloads, controlled restore, SQLite-to-PostgreSQL migration and application update UI remain absent. Snapshot/key checks do not establish remote Agent trust, source authenticity or restorability. | P1 |
| Notifications/renewals | First slice published on `main` at `bf8eaa8`: administrator Telegram configuration, preview/test, durable package-expiry reminders and Chinese delivery history; [operator guide](notifications.md) and [source-reviewed design](notifications-plan.md) describe the boundary. Backend 2298, frontend 890/72 files, both builds, real browser, working-tree and exact-commit Docker, and all four CI gates passed. Production was not upgraded and no real Telegram canary was run. | User binding, Bot/Mini App, daily digest, threshold/server/security alerts, announcements and renewal approval are absent. No payment or automatic extension. | P1 |
