...
| P1 | Administrator MFA | Candidate implements encrypted TOTP, one-use recovery codes, password-bound challenges, mandatory enrollment and local reset. Documented safety differences from the pinned source are explicit. | Passed 25 focused backend tests, frontend tests/builds and isolated production-browser workflows on 2026-08-30. Configure the persistent encryption key before use; multi-admin RBAC remains separate. |
...
| Account security | Subscriber sessions, TOTP, recovery codes and device revocation implemented; candidate adds [administrator MFA](administrator-security.md), mandatory enrollment, replay protection and local recovery | Multi-administrator RBAC, security events and IP-ban console absent. | P1 |
