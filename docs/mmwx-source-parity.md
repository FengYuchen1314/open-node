...
| --- | --- | --- | --- |
| Server/Agent inventory and telemetry | Implemented; [server sharing](server-sharing.md) has a separate owner/consumer first slice | DDNS, federated-server integration into ordinary inventory/node catalog, optional official federation crypto/reverse interop, home speed tester and Reality sharing remain absent. | P2, P1 for dependent workflows |
