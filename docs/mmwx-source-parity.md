...
| Setup/backup/database | CLI setup, installer-level stopped-volume backups, age tools, [consistent snapshots](backup-runtime.md) and [Web backup jobs/downloads plus offline v1 restore](backups.md) implemented. Restore publishes a new private directory, validates SQLite/key dependencies, invalidates old authority and work, isolates first boot, and requires Chinese administrator review plus explicit restart. A separate Compose template is included; real-age/SQLite/API and focused UI checks passed as recorded in [testing](testing.md). | Browser setup/upload restore, no-user initialization restore, legacy backup conversion, SQLite-to-PostgreSQL migration and application update UI remain absent. Source authenticity and remote Agent state/trust still require operator review. | P1 |
