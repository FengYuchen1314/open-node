...
| P1 | Custom rules, Proxy Providers and scripts | [Subscription customizations](subscription-customizations.md) implement owner-scoped DNS/rules/rule-providers CRUD, replace/prepend/append rendering, profile selection, missing policy groups, snapshot-backed Mihomo Providers and isolated QuickJS `post_fetch` / `pre_save_nodes` hooks with administrator and subscriber Chinese UI. | Server-side `mmw` processing, GeoIP filters, custom upstream headers and official on-disk application history remain. |
