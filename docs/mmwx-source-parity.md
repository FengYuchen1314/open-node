...
| Managed subscriptions | Users, plans, nodes, credentials, quota/reset, templates, profiles, temporary links, access reconciliation and source-compatible frozen accounting; [external sources](external-subscriptions.md) support administrator/subscriber manual and scheduled YAML/URI/Base64 workflows; [manual renewals](renewals.md) provide atomic review and extension | The provider/rule/script ecosystem remains incomplete. Renewal is not payment-provider integration. | P1/P2 |
...
| Setup/backup/database | [Browser first-run setup](initial-setup.md) now creates the first administrator and site titles with a locally issued 30-minute credential; default installer and renewal action are connected. CLI setup, installer-level stopped-volume backups, age tools, [consistent snapshots](backup-runtime.md), [Web backup jobs/downloads plus offline v1 restore](backups.md) are implemented. Restore validates data/keys, invalidates old authority and work, isolates first boot, and requires Chinese administrator review plus explicit restart. Targeted real CLI/SQLite/API, installer-function and frontend checks passed; see [testing](testing.md). | Setup profile/avatar/domain and PostgreSQL options, browser upload restore, no-user initialization restore and application update UI remain absent. Legacy conversion is out of scope. Source authenticity and remote Agent state/trust still require operator review. The new browser setup has not had a full Docker/browser deployment gate. | P1 |
| Notifications/renewals | Existing administrator Telegram notices remain available, without new Bot development. [User renewal requests and Web administrator review](renewals.md), frozen cycle days, cancellation and atomic extension are implemented. | Web announcements remain absent. Bot/Mini App, Telegram binding and Bot-only alerts are excluded by the current request. No payment integration or automatic renewal is claimed. | P1 for Web announcements; Bot out of scope |
| Dynamic system settings | Two-field site text is published on `main` at `f0ed515`. The current appearance slice adds SQLite-backed Logo/login-background URLs and bounded uploads, plus standard Ant Design light/dark/system themes, public projection, administrator CAS writes and Chinese controls. See the [site settings](system-settings.md), [appearance guide](appearance.md) and [pinned-source design](system-settings-plan.md). Targeted backend/frontend gates, type checks and the production build passed; production was not upgraded. | Announcements, silent mode, update channels, arbitrary feature flags and executable custom HTML/CSS/JavaScript remain absent. Probe settings stay independent and no license gate is added. | P1/P2 |
