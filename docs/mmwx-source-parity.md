...
| Setup/backup/database | CLI administrator creation and installer-level stopped-volume backup implemented; the [source-reviewed backup plan](backup-plan.md) records the next slice's consistency and restoration boundaries, not an implemented API | Browser setup, downloadable backup/restore, SQLite-to-PostgreSQL migration and application update UI absent. | P1 |
