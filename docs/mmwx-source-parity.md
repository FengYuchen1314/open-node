...
| --- | --- | --- | --- |
| Server/Agent inventory and telemetry | Implemented for local and federated servers. [DDNS](ddns.md) covers both; [server sharing](server-sharing.md) projects owner status, traffic, Xray/Nginx scan state and version, plus CPU/memory/disk/network telemetry into the ordinary inventory and node catalog. | The separate home speed tester and commercial Reality resource pool remain absent. | P2 |
