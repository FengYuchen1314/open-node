...
管理员可以在“系统设置”统一控制普通用户可见、可调用的四项可选功能：个人订阅
模板、外部订阅来源、个人路由节点和续费申请。订阅下载、套餐信息和账户安全始终
保留，管理员管理接口不受这项策略影响。

新安装默认开放四项功能。个人模板和外部订阅来源的数量上限默认都是 `0`，表示
不限制。关闭功能不会删除已有数据；重新开放后，用户可以继续使用原有数据。
...
用户中心读取 `/api/v1/account/permissions`，只显示已经开放的路由、模板、外部订阅
和续费入口，同时显示个人模板、外部订阅来源的已用数量及上限。权限读取失败时，
...
官方页面名与 Open Node 的产品结构不同：本项目把已经具备独立账户接口的模板、
