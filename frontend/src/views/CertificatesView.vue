...
import { certificateRequest, type CertificateCapabilities, type CertificateChallenge, type CertificateDetail, type DNSProvider, type ManagedCertificate } from "../services/certificates";
...
const capabilities = ref<CertificateCapabilities>({ available: false, directories: [], providers: [], challenge_types: [], webroots: [] });
...
<template>
  <div class="page-shell certificates-page">
    ...
    <section v-if="tab === 'certificates'" class="certificate-section">
      ...
      <article v-for="row in certificates" :key="row.id" class="certificate-row">
        ...
        <div class="certificate-actions">
          ...
          <v-btn :icon="row.version_id ? 'mdi-autorenew' : 'mdi-certificate-outline'" variant="text" size="small" :title="row.version_id ? 'Renew certificate' : 'Issue certificate'" :aria-label="row.version_id ? 'Renew certificate' : 'Issue certificate'" :disabled="busy || Boolean(row.active_job_id) || !row.directory_url || !capabilities.available" @click="queue(row)" />
    ...
    <section v-if="detail && tab === 'certificates'" class="certificate-detail">
      ...
      <div class="certificate-toolbar">
        <v-switch :model-value="detail.certificate.auto_renew" label="Auto-renew" color="primary" hide-details density="compact" :disabled="busy || !detail.certificate.directory_url" @update:model-value="toggleRenew(detail.certificate, $event)" />
        <v-checkbox v-model="force" label="Force renewal" hide-details density="compact" :disabled="busy || !detail.certificate.directory_url" />
        <v-btn icon="mdi-autorenew" title="Renew now" aria-label="Renew now" variant="text" :disabled="busy || !detail.certificate.version_id || !detail.certificate.directory_url || Boolean(detail.certificate.active_job_id) || !capabilities.available" @click="queue(detail.certificate, force)" />
    ...
    <v-dialog :model-value="Boolean(dialog)" max-width="620" scrollable :persistent="busy" @update:model-value="(value) => { if (!value) dialog = ''; }">
      <v-card class="certificate-dialog">
        ...
        <v-card-actions><v-spacer /><v-btn :disabled="busy" @click="dialog = ''">Cancel</v-btn><v-btn v-if="dialog === 'provider'" type="submit" form="provider-form" color="primary" :loading="busy" :disabled="!providerForm.name || !providerFields?.required.every((key) => providerForm.credentials[key])">Save provider</v-btn><v-btn v-else-if="dialog === 'certificate'" type="submit" form="certificate-form" color="primary" :loading="busy" :disabled="!canCreate">Create certificate</v-btn><v-btn v-else type="submit" form="import-form" color="primary" :loading="busy" :disabled="!importForm.name || !importForm.cert_pem || !importForm.key_pem">Import certificate</v-btn></v-card-actions>
      </v-card>
    </v-dialog>
  </div>
</template>
