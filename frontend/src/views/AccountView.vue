    ...
    <main class="account-content">
      ...
      <template v-if="tab === 'subscription' && profile && quota">
        ...
        <section class="account-links" aria-label="Subscription links">
          ...
          <div class="account-link-mode"><v-btn-toggle v-if="!selectedProfile && subscription?.short_links_enabled" v-model="linkType" mandatory density="compact" color="primary" variant="outlined" aria-label="Subscription link type"><v-btn value="full">Full</v-btn><v-btn value="short">Short</v-btn></v-btn-toggle><v-chip v-else size="small" variant="tonal" prepend-icon="mdi-link-variant">{{ selectedProfile ? 'MMWX profile' : 'Secure link' }}</v-chip><v-tooltip v-if="subscription?.short_links_enabled" text="Edit subscription short code"><template #activator="{ props: tip }"><v-btn v-bind="tip" icon="mdi-link-edit" aria-label="Edit subscription short code" variant="text" :disabled="loading || !subscription || Boolean(selectedProfile)" @click="shortCodeOpen = true" /></template></v-tooltip></div>
