...
describe("React dependent change sets", () => {
  it("loads after StrictMode remount rather than retaining the cancelled request's loading lock", async () => {
    ...
    expect((screen.getByRole("button", { name: "Dispatch" }) as HTMLButtonElement).disabled).toBe(false);
  });
  it("creates a complete routed plan and deduplicates excludes without inventing client settings", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Parent inbound tag"), { target: { value: " parent-vless " } });
    fireEvent.change(screen.getByLabelText("Label"), { target: { value: " direct-v4 " } });
    fireEvent.change(screen.getByLabelText("Sniffing excludes"), { target: { value: "Example.com,example.COM\nother.example" } });
    fireEvent.click(screen.getByRole("switch", { name: "Dispatch now" }));
    fireEvent.click(screen.getByRole("button", { name: "Create plan" })); await flush();
  ...
  it("validates required inbound and non-object outbound without sending commands", async () => {
    render(<ChangesView />); await flush(); fireEvent.click(screen.getByRole("button", { name: "Create plan" })); await flush();
    expect(screen.getByText("Inbound tag is required.")).toBeTruthy();
    fireEvent.change(screen.getByLabelText("Parent inbound tag"), { target: { value: "inbound" } });
    fireEvent.change(screen.getByLabelText("Outbound JSON"), { target: { value: "[]" } });
    fireEvent.click(screen.getByRole("button", { name: "Create plan" })); await flush();
    expect(screen.getByText("Outbound JSON must be a JSON object.")).toBeTruthy(); expect(createRoutedOutboundChangeSet).not.toHaveBeenCalled();
  });
  it("never clamps a negative timeout before validating a routed plan", async () => {
    render(<ChangesView />); await flush(); fireEvent.change(screen.getByLabelText("Parent inbound tag"), { target: { value: "parent" } });
    ...
    expect(screen.getByText("Timeout must be greater than zero.")).toBeTruthy();
  });
  it("keeps an incomplete timeout exponent invalid until its final digit is entered", async () => {
    render(<ChangesView />); await flush(); fireEvent.change(screen.getByLabelText("Parent inbound tag"), { target: { value: "parent" } }); const input = screen.getByLabelText("Timeout ms");
  ...
  it("retains raw forward/rollback steps and rejects invalid step shapes", async () => {
    render(<ChangesView />); await flush(); fireEvent.click(screen.getByRole("tab", { name: "Raw steps" }));
    const panel = within(screen.getByRole("tabpanel", { name: "Raw steps" }));
    fireEvent.change(panel.getByLabelText("Name"), { target: { value: " raw plan " } });
    fireEvent.change(panel.getByLabelText("Steps JSON"), { target: { value: '[{"server_id":"edge","forward":{"path":"/api/child/system/info"},"rollback":[]}]' } });
    fireEvent.click(panel.getByRole("button", { name: "Create" })); await flush();
    expect(screen.getByText("Step 1 rollback must be an object or null.")).toBeTruthy(); expect(createChangeSet).not.toHaveBeenCalled();
    const steps = [{ server_id: "edge", label: "Read", forward: { method: "GET", path: "/api/child/system/info" }, rollback: null }];
    fireEvent.change(panel.getByLabelText("Steps JSON"), { target: { value: JSON.stringify(steps) } });
    fireEvent.click(panel.getByRole("button", { name: "Create" })); await flush();
  ...
  it("does not allow a pre-dispatch list request to restore the old state", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Refresh change sets" })); await flush();
    fireEvent.click(screen.getByRole("button", { name: "Dispatch" })); await flush();
  ...
  it("keeps node reservations blocked until command results arrive", async () => {
    ...
    render(<ChangesView />); await flush();
    expect(screen.queryByRole("button", { name: "Accept current state" })).toBeNull();
    expect(screen.getByText("Waiting for command results: pending-compensation")).toBeTruthy();
    expect(screen.getByText("1 nodes reserved")).toBeTruthy();
    fireEvent.change(screen.getByLabelText("Rollback reason"), { target: { value: " retry verified " } });
    fireEvent.click(screen.getByRole("button", { name: "Retry rollback" })); await flush();
  ...
  it("requires both acceptance reason and explicit acknowledgement", async () => {
    ...
    render(<ChangesView />); await flush(); fireEvent.click(screen.getByRole("button", { name: "Accept current state" }));
    const dialog = within(screen.getByText("Accept current state", { selector: ".ant-modal-title" }).closest('[role="dialog"]')!);
    expect((dialog.getByRole("button", { name: "Accept state" }) as HTMLButtonElement).disabled).toBe(true);
    fireEvent.change(dialog.getByLabelText("Resolution reason"), { target: { value: "  Verified nodes  " } });
    expect((dialog.getByRole("button", { name: "Accept state" }) as HTMLButtonElement).disabled).toBe(true);
    fireEvent.click(dialog.getByRole("checkbox", { name: "I have checked the nodes and accept any remaining changes" }));
    fireEvent.click(dialog.getByRole("button", { name: "Accept state" })); await flush();
    expect(acceptChangeSet).toHaveBeenCalledWith("plan", "Verified nodes");
    expect(screen.getByText("Current state accepted. Node reservations released.")).toBeTruthy();
  });
  it("locks all mutation actions for archived steps", async () => {
    ...
    render(<ChangesView />); await flush();
    expect(screen.getByText("Deleted server (archived)")).toBeTruthy();
    expect((screen.getByRole("button", { name: "Dispatch" }) as HTMLButtonElement).disabled).toBe(true);
