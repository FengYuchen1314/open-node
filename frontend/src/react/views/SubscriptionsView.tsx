      ...
      <Col xs={24} xl={11}><Card title="工作流程" extra={loading && <Spin size="small" />}><Typography.Paragraph type="secondary">用户、托管节点、套餐与分配</Typography.Paragraph>
        <Tabs activeKey={activeTab} onChange={setActiveTab} items={[
          ...
          { key: "plans", label: "套餐", children: <Form layout="vertical" preserve={false} disabled={disabled} onFinish={submitPlan}>
            ...
            <Form.Item label="套餐节点" required help={!planForm.node_ids.length ? "必须选择至少一个节点；勾选即可，无需按 Ctrl。" : `已选择 ${planForm.node_ids.length} 个节点，这些节点会写入套餐订阅。`}>
              <Select aria-label="套餐节点" mode="multiple" showSearch placeholder="勾选套餐中可用的节点" options={nodeOptions} value={planForm.node_ids} onChange={node_ids => patchPlan({ node_ids })} />
            </Form.Item>
            {!nodeOptions.length && <Alert type="warning" showIcon style={{ marginBottom: 16 }} title="当前没有可加入套餐的节点" description="请先在“节点管理”中创建并启用节点，再回来创建套餐。" />}
