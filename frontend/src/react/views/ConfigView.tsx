...
export default function ConfigView(props: ConfigViewProps) {
  ...
  const isFederated = selectedServer?.is_federated === true;
  ...
  function selectServer(id: string, inventory = serversRef.current) {
    ...
    epoch.current += 1;
    setSelectedId(id);
    if (inventory.find((server) => server.id === id)?.is_federated) setActiveTab("runtime");
  ...
  const runtimeTab = <Space orientation="vertical" size="large" style={{ width: "100%" }}>
    ...
    {!isFederated && <>{deploymentForm}{chainForm}</>}
    ...
    {!isFederated && <Card size="small" title="运行时隧道">
  ...
  return <Space orientation="vertical" size="large" style={{ width: "100%" }}>
    ...
    <Card title="工作区"><Typography.Paragraph type="secondary">{isFederated ? "分享服务器节点与凭据工作区" : "MMW Agent 子接口配置操作"}</Typography.Paragraph><Form layout="vertical"><Form.Item label="目标服务器"><Select aria-label="目标服务器" value={selectedId || undefined} options={serverOptions} disabled={!serverOptions.length} onChange={(id) => selectServer(id)} /></Form.Item></Form><Tabs activeKey={activeTab} onChange={setActiveTab} items={isFederated ? [
      { key: "runtime", label: "运行时", children: runtimeTab },
    ] : [
