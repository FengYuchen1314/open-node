...
def exercise(work, fixture, args, operator, backend, endpoint, ca):
    ...
    with native.echo_server(work) as (echo, _), sync_playwright() as playwright:
        ...
        administrator = browser.new_context(viewport={"width": 1440, "height": 1000})
        primary = browser.new_context(
            viewport={"width": 1440, "height": 1000},
            permissions=["clipboard-read", "clipboard-write"],
        )
        secondary = browser.new_context(viewport={"width": 390, "height": 844})
        errors, requests = [], []
        ...
        try:
            ...
            admin.get_by_role(
                "button", name="Login settings for alice", exact=True
            ).click()
            dialog = admin.get_by_role("dialog")
            dialog.get_by_label("New login password", exact=True).fill(password)
            dialog.get_by_label("Confirm login password", exact=True).fill(password)
            dialog.get_by_label("Revoke all existing user sessions", exact=True).check()
            capture(admin, args.output, "admin-login")
            dialog.get_by_role("button", name="Save password", exact=True).click()
            expect(
                dialog.get_by_text(
            ...
            with page.expect_download() as downloaded:
                page.get_by_label("Download subscription", exact=True).click()
            ...
            key = dialog.get_by_label("Setup key", exact=True)
            expect(key).to_be_visible()
            secret = key.input_value()
            image = dialog.get_by_alt_text("Authenticator enrollment QR code")
            ...
            dialog.get_by_label("Authenticator code", exact=True).fill(used_code)
            dialog.get_by_role("button", name="Verify and enable", exact=True).click()
            expect(dialog.get_by_text("Recovery codes", exact=True)).to_be_visible()
            with page.expect_download() as codes_download:
                dialog.get_by_role(
                    "button", name="Download recovery codes", exact=True
                ).click()
            ...
            dialog.get_by_label(
                "I have stored my recovery codes securely", exact=True
            ).check()
            dialog.get_by_role("button", name="Done", exact=True).click()
            other.reload()
            expect(
                other.get_by_role("heading", name="Subscriber Sign-In", exact=True)
            ).to_be_visible()
            sign_in(other, backend, password)
            expect(
                other.get_by_role("heading", name="Two-Factor Verification", exact=True)
            ).to_be_visible()
            assert secondary.request.get(backend + ACCOUNT + "/me").status == 401
            other.get_by_label("Authenticator or recovery code", exact=True).fill(
                used_code
            )
            other.get_by_role("button", name="Verify", exact=True).click()
            expect(other.get_by_text("Invalid credentials", exact=True)).to_be_visible()
            other.get_by_label("Authenticator or recovery code", exact=True).fill(
            ...
            expect(
                page.get_by_role("button", name="Revoke others", exact=True)
            ).to_be_enabled()
            capture(page, args.output, "sessions")
            page.get_by_role("button", name="Revoke others", exact=True).click()
            other.reload()
            expect(
                other.get_by_role("heading", name="Subscriber Sign-In", exact=True)
            ).to_be_visible()
            ...
            dialog = page.get_by_role("dialog")
            dialog.get_by_label("Current password", exact=True).fill(password)
            dialog.get_by_label("New password", exact=True).fill(replacement)
            dialog.get_by_label("Confirm password", exact=True).fill(replacement)
            dialog.get_by_label("Authenticator or recovery code", exact=True).fill(
                codes[1]
            )
            ...
            dialog = page.get_by_role("dialog")
            dialog.get_by_label("Current password", exact=True).fill(replacement)
            dialog.get_by_label("Authenticator or recovery code", exact=True).fill(
                codes[3]
            )
            dialog.get_by_role("button", name="Confirm", exact=True).click()
            expect(
                page.get_by_text("Subscription links reset", exact=True)
            ).to_be_visible()
            ...
            dialog.get_by_label("Confirm login password", exact=True).fill(password)
            dialog.get_by_label(
                "Reset two-factor authentication and recovery codes", exact=True
            ).check()
            dialog.get_by_label("Revoke all existing user sessions", exact=True).check()
            dialog.get_by_role("button", name="Save password", exact=True).click()
            expect(
