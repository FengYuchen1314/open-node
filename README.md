...
管理端侧边栏提供七个工作区：服务器管理、节点管理、模板管理、套餐管理、
用户管理、证书管理和系统设置。节点创建/编排、套餐绑定节点/模板、用户绑定套餐各自在对应
工作区完成；网站反向代理使用的可信证书可在证书管理中签发并部署到目标服务器。
...
- “节点编排”页面可把候选节点拖成从左到右的多跳线路。线路至少 2 跳、最多 8 跳；
  ...
  节点重复、同一服务器再次经过以及回还环路。候选中同时包含受管节点和已确认的外部节点；
  可组建纯外部或混合线路，但一条线路内的外部节点必须属于同一订阅用户，也只能分配给该用户。
  Clash/Stash 使用轮询组，Xray 使用原生 `roundRobin` balancer；sing-box 1.14 没有轮询组，
  因此明确降级为标准 `urltest` 自动低延迟/故障切换，不生成虚假策略。
